...
const NowPlayingfun = (movie) => {
    ...
    <a class="posterlink" href=${url}> <img class="poster" data-id="${
        ...
        <div class="date_rating">
            ...
            <p class="rating">${
							movie.vote_average
						}<span><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="Yellow" class="star bi-star-fill" viewBox="0 0 16 16">
...
TodayTrending().then((trends) => {
	...
	const goToSlide = function (slides) {
		slide.forEach(
			(item, i) =>
				(item.style.transform = `translateX(${100 * (i - slides)}%)`),
		);
	};
